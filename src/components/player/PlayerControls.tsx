...
import { Maximize, Pause, Play, RotateCcw, Volume1, Volume2, VolumeX } from "lucide-react";
...
export function PlayerControls({ channel, onFullscreen, onPlayPause, onRetry, onToggleMute, onTogglePiP, onVolumeChange }: PlayerControlsProps) {
      ...
      <div className="pointer-events-auto rounded-2xl border border-white/12 bg-slate-950/50 p-2 shadow-[0_20px_60px_rgba(2,8,23,0.28)] backdrop-blur-xl light:bg-white/72">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <div className="min-w-0">
            <p className="text-xs uppercase tracking-[0.18em] text-sky-200 light:text-sky-700">{status === "loading" ? "Cargando" : status === "error" ? "Error" : "Canal seleccionado"}</p>
            <h3 className="truncate text-sm font-semibold text-white light:text-slate-950">{channel?.name ?? "Selecciona un canal para empezar"}</h3>
          </div>
